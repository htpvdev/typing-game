...
document.querySelector<HTMLDivElement>('#app')!.innerHTML = `
  <div>
    ...
    <div id="genreSelection">
      <button id="iphoneGenreBtn">通信</button>
      <button id="programGenreBtn">プログラミング</button>
    </div>
...
const questionViewEl = document.querySelector<HTMLDivElement>('#questionView')?.querySelector('p');
// const furiganaViewEl = document.querySelector<HTMLDivElement>('#furiganaView');
const inputKeyViewEl = document.querySelector<HTMLDivElement>('#inputKeyView')?.querySelector('p');
const resultViewEl = document.querySelector<HTMLDivElement>('#resultView')?.querySelector('p');
const userInputEl = document.querySelector<HTMLInputElement>('#userInput');
const countdownEl = document.querySelector<HTMLDivElement>('#countdown')?.querySelector('p');
const gameAreaEl = document.querySelector<HTMLDivElement>('#gameArea');
const genreSelectionEl = document.querySelector<HTMLDivElement>('#genreSelection');
...
const changeWord = () => {
  if (currentQuestion < setQuestions.length) {  // 修正：問題数に基づく条件判定
    ...
    setHtml(inputKeyViewEl, returnInputText(setQuestions[currentQuestion].furigana))
    // console.log('changeWord, next text is ', returnInputText(setQuestions[currentQuestion].furigana));
    currentQuestion++;
  } else {
    setStyle(gameAreaEl, 'none')
    setHtml(resultViewEl, `単語数: ${correctCount} / ${setQuestions.length}<br>間違えた文字数: ${wrongCharCount}`)
  }
}
...
const startGame = (genre: 'iphone' | 'program') => {
  ...
  setStyle(genreSelectionEl, 'none')
...
const restartGame = () => {
  ...
  setStyle(genreSelectionEl, 'flex')
  setStyle(gameAreaEl, 'none')
...
document.querySelector<HTMLButtonElement>('#iphoneGenreBtn')?.addEventListener('click', () => startGame('iphone'));
document.querySelector<HTMLButtonElement>('#programGenreBtn')?.addEventListener('click', () => startGame('program'));
document.querySelector<HTMLButtonElement>('#restartBtn')?.addEventListener('click', restartGame);
...
userInputEl?.addEventListener('keydown', (e) => {
  ...
  if (inputKey === currentTarget[0]) {
    ...
    setHtml(msgEl, `<span style="color: red">${randomMsg(wrongCharCount)}</span>`)
